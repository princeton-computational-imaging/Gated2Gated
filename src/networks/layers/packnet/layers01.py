...
class Conv2D(nn.Module):
    ...
    stride : int
        Stride
    """
    def __init__(self, in_channels, out_channels, kernel_size, stride, use_batchnorm=False):
        super().__init__()
...
class ResidualConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, dropout=None, use_batchnorm=False):
        """
...
def ResidualBlock(in_channels, out_channels, num_blocks, stride, dropout=None, use_batchnorm=False):
    """
    ...
    dropout : float
        Dropout value
    """
    layers = [ResidualConv(in_channels, out_channels, stride, dropout=dropout,use_batchnorm=use_batchnorm)]
...
class InvDepth(nn.Module):
    """Inverse depth layer"""
    def __init__(self, in_channels, out_channels=1, min_depth=0.5, activation = 'sigmoid'):
        """
        ...
        self.activ = activations[activation]    # changed it to dict because for albedo and ambient should use linear activations
