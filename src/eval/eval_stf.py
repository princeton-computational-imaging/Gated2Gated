...
def evaluate(opt):
    ...
    with open("splits/gated/val_files_ids.txt", "r") as f:
        val_ids = f.read().split('\n')

    lidar_paths = [os.path.join(dataset_dir,"lidar_hdl64_last_gated","{}.npz".format(_id)) for _id in val_ids]

    # gated_ids = [os.path.splitext(os.path.basename(item))[0] for item in lidar_paths]
    gated_paths = [os.path.join(dataset_dir,"gated{}","{}.tiff".format(_id)) for _id in val_ids]    
    ...
    with torch.no_grad():
        for lidar_path,gated_path in tzip(lidar_paths,gated_paths):
            ...
            if opt.gen_figs:    
                ...
                depth_lidar1_color = visualize2D.colorize_pointcloud(gt_depth, min_distance=MIN_DEPTH,max_distance=MAX_DEPTH, radius=3, cmap=cmap_dict[opt.cmap])

                #Generate colorized depth map
                depth_map_color = visualize2D.colorize_depth(pred_depth, min_distance=MIN_DEPTH, max_distance=MAX_DEPTH, cmap=cmap_dict[opt.cmap])
...
if __name__ == "__main__":
    ...
    options.add_argument("--cmap",default='jet',
                         choices=['jet','jet_r','plasma','plasma_r','magma', 'magma_r'],   
